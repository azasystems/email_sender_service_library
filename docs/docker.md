[Source of the docs:](https://github.com/ineat/docker-php-nginx-postgres-composer)

# docker-php-nginx-postgres-composer
Docker Compose configuration to run PHP with Nginx, PHP-FPM, PostgreSQL and Composer.

## Overview

Docker Compose configuration lets you run easily PHP with Nginx, PHP-FPM, PostgreSQL and Composer.
It exposes 4 services:

* web (Nginx)
* php (PHP with PHP-FPM)
* db (PostgreSQL)
* composer
...
Nginx will be available on `localhost:85` and PostgreSQL on `localhost:5432`.
